  ...
  return (
    <main className="flex flex-col min-h-screen p-4">
      ...
      <div className="flex-grow flex flex-col items-center justify-center w-full max-w-sm mx-auto text-center pb-24 lg:pb-44">
        ...
        <p className="text-lg mb-8">to continue to your Context Layers account</p>
        {providers.map((provider) => (
          <button
            key={provider.id}
            onClick={() => signIn(provider.id, { callbackUrl: "/" })}
            className="relative w-full flex items-center justify-center rounded-lg border border-primary/30 bg-transparent font-medium py-3 px-4 mb-4 hover:bg-muted transition-colors duration-300 ease-in-out"
          >
            {provider.icon}
